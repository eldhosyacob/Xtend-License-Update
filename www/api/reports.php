...
if (isset($_GET['summary']) && $_GET['summary'] === 'true') {
  try {
    ...
    $stmt = $db->prepare("SELECT COUNT(*) FROM license_details WHERE licensee_validtill >= :today");
    $stmt->bindValue(':today', $today);
    $stmt->execute();
    $activeCount = $stmt->fetchColumn();

    // Expired Licenses (valid_till < today)
    $stmt = $db->prepare("SELECT COUNT(*) FROM license_details WHERE licensee_validtill < :today AND licensee_validtill != '' AND licensee_validtill IS NOT NULL");
    $stmt->bindValue(':today', $today);
    $stmt->execute();
    $expiredCount = $stmt->fetchColumn();

    // Expiring Soon (today <= valid_till <= next30Days)
    $stmt = $db->prepare("SELECT COUNT(*) FROM license_details WHERE licensee_validtill >= :today AND licensee_validtill <= :next30Days");
    $stmt->bindValue(':today', $today);
    ...
    // Total Licenses
    $stmt = $db->query("SELECT COUNT(*) FROM license_details");
...
try {
  ...
  $sql = "SELECT id, created_on, licensee_dealer, location_name, location_code, licensee_validtill, system_serialid, system_uniqueid, engine_graceperiod, device_status 
